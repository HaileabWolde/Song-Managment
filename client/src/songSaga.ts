...
function* workGetAllSongFetch(): Generator<any, void, any> {
  const Songs =  yield call(() => fetch('https://backend-0jtt.onrender.com/Songs/AllSong'));
...
function* workGetStatics() : Generator<any, void, any> {
  const Statics =  yield call(() => fetch('https://backend-0jtt.onrender.com/Songs/statics'));
...
function* workDeleteSong(action: PayloadAction<string>): Generator<any, void, any> {
  ...
  const endpoint = `https://backend-0jtt.onrender.com/Songs/deleteSong/${songId}`;
...
function*  workEditSong(action: PayloadAction<{ id: string; data: FormData }>): Generator<any, void, any> {
  ...
  const endpoint = `https://backend-0jtt.onrender.com/Songs/updateSong/${id}`;
  try {
    const res = yield call(fetch, endpoint, {
      method: 'PUT',
      headers: {
...
function* SearchSong (action: PayloadAction<string>): Generator<any, void, any> {
  ...
  const endpoint = `https://backend-0jtt.onrender.com/Songs/search?searchInfo=${Data || 'none'}`;
  try {
    const res = yield call(fetch, endpoint, {
      method: 'GET',
      headers: {
...
function* workCreateSong(action: PayloadAction<{ data: FormData }>):Generator<any, void, any> {
  ...
  const endpoint = `https://backend-0jtt.onrender.com/Songs/createSong`;
  try {
    const res = yield call(fetch, endpoint, {
      method: 'POST',
      headers: {
